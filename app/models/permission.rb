...
class Permission < ValueObject
  ...
  # Define constants for common actions, and for resources (business objects).
    # These constants represent various actions a user can perform, and different business objects in the application.
  #
  READ                     = 'read'
  WRITE                    = 'write'
  ...
  # TODO Define Extra Constant for Actions/Resources for 'Preventions' here, If needed
    # Action Constants
  #

  # Define mapping of actions to resources. This provides a way to check what actions are allowed for each resource.
  RESOURCE_ACTIONS = {
    CASE => [
    ...
    # NOTE Added all the common Constants that are in CASE, INCIDENT, and TRACING_REQUEST.
    # TODO Ensure that these are the needed Action Constants for Prevention
    PREVENTION => [
      READ, CREATE, WRITE, ENABLE_DISABLE_RECORD, FLAG, EXPORT_LIST_VIEW, EXPORT_CSV, EXPORT_EXCEL, EXPORT_PDF,
      EXPORT_JSON, EXPORT_CUSTOM, IMPORT, CHANGE_LOG, MANAGE
    ],
    ...
    DASHBOARD => [
      ...
      DASH_VIOLATIONS_CATEGORY_REGION, DASH_PERPETRATOR_ARMED_FORCE_GROUP_PARTY_NAMES
    ],
  ...
  # Class constructor to initialize permission objects.
  def initialize(args = {})
    super(args)
  end

  # Class methods to retrieve available permissions and resources.
  class << self
    ...
    # Return a list of all available actions across resources.
    def actions
      Permission::RESOURCE_ACTIONS.values.flatten.uniq
    end

    # Return a list of available resources (business objects).
    def resources
      [
        CASE, INCIDENT, TRACING_REQUEST, PREVENTION, REPORT,
        POTENTIAL_MATCH, REGISTRY_RECORD, ROLE, USER, USER_GROUP, AGENCY, WEBHOOK,
        METADATA, SYSTEM, MANAGED_REPORT, DASHBOARD, AUDIT_LOG, MATCHING_CONFIGURATION, DUPLICATE,
        CODE_OF_CONDUCT, ACTIVITY_LOG
      ]
    end

    # Return a list of all Records related Permissions
    def records
      [
        CASE           ,
        INCIDENT       ,
    ...
    # Return a list of all Management related Permissions
    def management
      [SELF, GROUP, ALL, ADMIN_ONLY, AGENCY, WEBHOOK]
    end

    # Return a list of all permissions (actions + resources).
    def all
      actions + resources + management
    end
    alias all_permissions all

    # Return permissions grouped by 'actions,' 'resource,' and 'management.'
    def all_grouped
      { 'actions' => actions, 'resource' => resources, 'management' => management }
    end

    # Return a list of permissions with available resource-action combinations.
    def all_available
  ...
  # Retrieve the corresponding class for a resource
    # (e.g., Case, Incident, Tracing Request, Prevention, Registry Record).
  #
  def resource_class
