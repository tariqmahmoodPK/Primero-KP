...
Rails.application.routes.draw do
  # Root route to the 'home#v2' controller action
  root to: 'home#v2'

  # Scope for version 2 ('v2')
  scope :v2 do
    get '/'   , to: 'home#v2' # Match the root route for version 2
    get '*all', to: 'home#v2' # Match all other routes under version 2
  end

  # Devise routes for user authentication and sessions
  devise_for(
    :users             ,
    class_name:  'User',
    path:        '/api/v2/tokens',
  ...
  # Devise scope for user-related actions
  devise_scope :user do
    # Routes for password reset, and password reset request in version 2
    get '/v2/password_reset'        , to: 'home#v2', as: :password_reset
    get '/v2/password_reset_request', to: 'home#v2', as: :password_reset_request
  end

  # Resource routes for 'health' and 'login' controllers
  resources :health , only: %i[index show]
  resources :login  , only: [:show]

  # Namespace for API version 2 with JSON format and constraints
  namespace :api do
    namespace :v2, defaults: { format: :json }, constraints: { format: :json }, only: %i[index create show update destroy] do
      # Resource routes for 'primero' controller
      resources :primero, only: %i[index]

      # Resource routes for 'preventions' controller
      resources :preventions

      # Resource routes for 'children' controller with alias 'cases'
      ...
      # Resource routes for 'form_sections' with alias 'forms'
      ...
      resources :users do
        # Custom route for 'password-reset-request'
        post :'password-reset-request', to: 'password_reset#user_password_reset_request'

        collection do
          # Custom routes for user transitions
      ...
      # Resource routes for 'identity_providers'
      resources :identity_providers, only: [:index]

      # Resource routes for 'dashboards'
      ...
      # Resource routes for 'managed_reports'
      ...
      # Resource routes for 'registry_records' with nested 'flags' and 'alerts'
      resources :registry_records do
        resources :flags , only: %i[index create update]
        resources :alerts, only: [:index]

        # Custom route for 'record_history'
        get :record_history, to: 'record_histories#index'
      end
    end
  end
